    ...
    def Renseigner_Exigence(self):
        ...
        def DemanderExigenceMere():
            if int(self.mere.get()) == False:
                for i in self.MgrExigences.read():
                    tk.Radiobutton(self, text=str(i.idex), variable=self.exigence_mere, value=i.idex).grid()
        ...
        def Valider():
            ...
            for i in self.MgrBesoins.read():
                tk.Radiobutton(self, text=str(i.id_besoin), variable=self.besoin_origine, value=i.id_besoin).grid(
                    column=1)
